Selenium.prototype.doRandomValue = function( options, varName ) {
	...
	switch( type ) {
	    case 'bool'         : storedVars[ varName ] = chance.bool(); break;
	    
        case 'float'        : storedVars[ varName ] = chance.floating(); break;
        case 'integer'      : storedVars[ varName ] = chance.integer(); break;
        case 'natural'      : storedVars[ varName ] = chance.natural(); break;        
        case 'maxint'       : storedVars[ varName ] = randomMaxInt( length ); break;
        case 'minint'       : storedVars[ varName ] = randomMinInt( length ); break;
        
        case 'alpha'        : storedVars[ varName ] = chance.string(); break;
        case 'character'    : storedVars[ varName ] = chance.character(); break;
        case 'word'         : storedVars[ varName ] = chance.word(); break;
        case 'paragraph'    : storedVars[ varName ] = chance.paragraph(); break;        
        case 'alphanumeric' : storedVars[ varName ] = randomAlphaNumeric( length ); break;
        case 'randhex'      : storedVars[ varName ] = randomHex(); break;
        
        case 'domain'       : storedVars[ varName ] = chance.domain(); break;
        case 'email'        : storedVars[ varName ] = chance.email(); break;
        case 'fbid'         : storedVars[ varName ] = chance.fbid(); break;
        case 'google_analyt': storedVars[ varName ] = chance.google_analytics(); break;
        case 'hashtag'      : storedVars[ varName ] = chance.hashtag(); break;
        case 'ip'           : storedVars[ varName ] = chance.ip(); break;
        case 'ipv6'         : storedVars[ varName ] = chance.ipv6(); break;
        
        case 'address'      : storedVars[ varName ] = chance.address(); break;
        case 'phone'        : storedVars[ varName ] = chance.phone(); break;
        case 'postal'       : storedVars[ varName ] = chance.postal(); break;
        case 'province'     : storedVars[ varName ] = chance.province(); break;
        case 'date'         : storedVars[ varName ] = chance.date(); break;
        case 'hammertime'   : storedVars[ varName ] = chance.hammertime(); break;
        ...
        case 'randarray'    : storedVars[ varName ] = randomArray(); break;        
        case 'javascript'   : storedVars[ varName ] = javascript(); break;
        default             : storedVars[ varName ] = chance.bool();
    };    
};
...
function randomMaxInt ( length ) {
    var maxInt = Number.MAX_INT + length;
    return maxInt;
}


function randomMinInt ( length ) {
    var minInt = -(Number.MAX_INT) - length;
    return minInt;
}
...
function randomArray() { 
	...
	var randArray = [-20000,0,1,10000,1500000];	  
	var currentIndex = randArray.length;  
	var randomIndex;
	var tempValue;	  
	...
	if (Math.random() > 0.5) {	    
		while (0 !== currentIndex) {
			...
			currentIndex -= 1;
			tempValue = randArray[currentIndex];
			randArray[currentIndex] = Math.random() * randArray[randomIndex];
	    	randArray[randomIndex] = tempValue;
	    }
		return "["+ randArray + "]";
	}
	else {		
...
function randomHex(){
    ...
    randHex = "";
    for (i = 0; i < 6; i++){
        digit[i] = hexArray[Math.round(Math.random()*14)];
        randHex = randHex + digit[i];
    }
    return randHex;
}
...
(function () {
    ...
    Chance.prototype.euro = function (options) {
        // By default, a somewhat more sane max for euro than all available numbers
        options = initOptions(options, {max : 10000, min : 0});

        var euro = this.floating({min: options.min, max: options.max, fixed: 2}).toString(),
            cents = euro.split('.')[1];
